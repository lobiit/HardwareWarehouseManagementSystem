...
class Program
{
    ...
    private static void ProcessItems(CustomQueue<HardwareItem> hardwareItemQueue)
    {
        ...
        {
            System.Threading.Thread.Sleep(3000);
            HardwareItem hardwareItem = hardwareItemQueue.GetItem();
        }
    }
    private static void CustomQueue_CustomQueueEvent(CustomQueue<HardwareItem> sender, QueueEventArgs eventArgs)
    {
        ...
        if (sender.QueueLength > 0)
        {
            ...
            WriteValuesInQueueToScree(sender);
    ...
    private static void WriteValuesInQueueToScree(CustomQueue<HardwareItem> hardwareItems)
    {
        foreach (var hardwareItem in hardwareItems)
        {
            Console.WriteLine($"{hardwareItem.Id, -6} {hardwareItem.Name, -15} {hardwareItem.Quantity, -10} {hardwareItem.Type, -20} {hardwareItem.UnitValue, 10}");
        }
    }
